...
class SimpleClock: ScreenSaverView {
	...
	override func draw(_ screenRect: NSRect) {
		...
		drawClockHand( // Second
			CGSize(width: getSize(skin.scaling.secondHandWidth),
                ...
                handCircleSize: CGSize(
                    width: getSize(skin.scaling.secondHandPointRadius), height: getSize(skin.scaling.secondHandPointRadius)
                ),
	...
	func drawClockHand(_ size: CGSize, overhangSize: CGSize, color: CGColor, lineCap: (CGLineCap, CGLineCap), progress: CGFloat, total: Int) {
        ...
        context.setShadow(offset: CGSize(width: 0, height: 0), blur: 1, color: skin.color.handShadow)
    ...
    func drawClockHand(_ size: CGSize, overhangSize: CGSize, handCircleSize: CGSize, color: CGColor, lineCap: (CGLineCap, CGLineCap), progress: CGFloat, total: Int) {
        let center: CGPoint = CGPoint(x: clockRect.midX, y: clockRect.midY)
        ...
        let handLineCircleX: CGFloat = center.x + (cos(angle) * size.height) - handCircleSize.height / 2
        let handLineCircleY: CGFloat = center.y + (sin(angle) * size.height) - handCircleSize.height / 2
        let rect = newRectCenteredInRect(handCircleSize, x: handLineCircleX, y: handLineCircleY)
...
func newRectCenteredInRect(_ size: CGSize, x: CGFloat, y: CGFloat) -> CGRect {

    var innerRect = NSRect()
    innerRect.size.width = size.width
    innerRect.size.height = size.height

    innerRect.origin.x = x
    innerRect.origin.y = y
